...
```{r, echo = FALSE}
# extract code from SOM file into a new R file
...
opts_chunk$set(echo = FALSE, 
               ...
               dpi = 300,
               fig.show = 'hide')
invisible(purl("Marwick_Hayes_et_al_SOM.Rmd",  
               output="temp",
               quiet=TRUE))
read_chunk("temp")
# now read relevant chunks of code from SOM...
```

```{r, ref.label = "hormodelsdiagnostics", fig.show = 'hide', results = 'hide'}
```

```{r, ref.label = "hordirmodelsdiagnostics",  fig.show = 'hide', results = 'hide'}
```

```{r, ref.label = "vermodelsdiagnostics",  fig.show = 'hide', results = 'hide'}
```

```{r, ref.label = "orientmodelsdiagnostics",  fig.show = 'hide', results = 'hide'}
```

```{r, ref.label = "plungemodelsdiagnostics",  fig.show = 'hide', results = 'hide'}
```
...
```{r}
# finished working with chunks from SOM so let's
# delete the temp file we made from the SOM file
unlink("temp")
```
...
In our experiments artefact shape variables were significant predictors for horizontal distance (shape and elongation have a positive relationship) and unsigned vertical distance (shape has a positive relationship, flatness has a negative relationship). Artefacts with high shape and elongation values are more discoidal, and artefacts with high shape and low flatness values are either discoidal or blade-like, depending on their elongation values (Figure \@ref(fig:zinggplotofartefacts)). These results differ from Barton and Bergman's [-@BartonBergman1982] observations that wider, flatter (i.e. discoidal) artefacts moved less. However, the adjusted R^2^ values for our models range between 0.1 and 0.3, indicating that although shape variables have a statistically significant role in influencing artefact movement during trampling, they have limited value for making specific predictions how far artefacts with certain shape attributes have moved due to prehistoric trampling. 

## Simulation of long-term tramplin by resampling

A common limitation of experimental studies of taphonomic processes such as trampling is the short time scale of experimental observations relative to the long time scales often represented by archaeological sites. The duration of our experiment was particularly short due to the unique use of the archaeological sediments in the spoil heap as the trampling substrate. In general, we lack experimental data on what patterns might emerge after years of occasasional trampling that might result from, for example, seasonal use of a rockshelter. To explore what might have happened if our experiment had run for a long time, we can simulate the effects of a large number of trampling events by resampling many times from the observed trampling event measurements. Yorsten et al. [@yorston1990simulation] describe this approach applied to the study the effect of ploughing on spatial distributions of ceramic sherds. They observed sherd locations at the Butser Ancient Farm Research Project over six years, fit a theoretical distribution to the distribution of experimental data, and evaluated the function at 10, 50, and 100 years. 


 Simulations such as this are a trade-off between realism and [@Aldenderfer1991; ]

cite:  @kohler2015review lake2014trends

For each artefact, we computed the difference in co-ordinates of the artefact's location after each of the three trampling events we recorded. The difference in co-ordinates are the co-ordinates of an artefact's starting location minus the co-ordinates of its final position, after the trampling event. To simulate artefact movement by trampling, we take each artefact in our sample and we randomly select one of its three observed differences in co-ordinates, and compute a new position for the artefact using these randomly selected co-ordinates. Then at this new position, we make another random selection from the three co-ordinate pairs, and add these co-ordinates to compute a new position. In each case we are taking the observed measurements specific to each artefact, and using them to compute new theoretical positions after a simulated trampling event. If we repeat this many times, we can simulate the movement of artefacts after a large number of events, which is challenging to do empirically.    
